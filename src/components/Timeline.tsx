...
export default function Timeline() {
  ...
  return (
    <section ref={containerRef} id="timeline" className="py-24 px-6 bg-transparent border-t border-slate-200 dark:border-white/5 relative overflow-hidden select-none">
      
      <div className="w-full max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <span className="font-mono text-xs text-blue-500 dark:text-blue-400 tracking-[0.2em] uppercase mb-4 block">// track records</span>
        ...
        <div className="relative max-w-3xl mx-auto mt-12 pl-8 md:pl-0">
          
          {/* Timeline central line track */}
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-[2px] bg-slate-200 dark:bg-white/5 -translate-x-1/2 pointer-events-none" />

          {/* Animated scrolling glowing line on top of track */}
          <motion.div
            style={{ scaleY }}
            className="absolute left-4 md:left-1/2 top-0 bottom-0 w-[2px] bg-gradient-to-b from-blue-500 via-violet-500 to-green-400 origin-top -translate-x-1/2 pointer-events-none"
          />

          {milestones.map((milestone, idx) => {
            const isEven = idx % 2 === 0;
            return (
              <div key={idx} className="relative mb-16 md:mb-24 flex flex-col md:flex-row items-start md:items-center">
                
                {/* Visual node on timeline line */}
                <div className="absolute left-4 md:left-1/2 -translate-x-1/2 z-10 w-10 h-10 rounded-full bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-800 dark:text-white shadow-md dark:shadow-xl">
                  {milestone.icon}
                </div>
                ...
                <div className={`w-full md:w-1/2 flex ${isEven ? "md:justify-end md:pr-14" : "md:justify-start md:pl-14 md:order-2"}`}>
                  <motion.div
                    ...
                    className="p-6 rounded-3xl border bg-slate-900/[0.03] dark:bg-white/[0.03] backdrop-blur-md transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-black/20 dark:hover:shadow-black/40 border-slate-200 dark:border-white/10 select-none max-w-md"
                  >
                    <span className="font-mono text-[10px] text-slate-400 dark:text-white/40 font-bold block mb-1 uppercase tracking-widest">
                      {milestone.year}
                    </span>
                    <h3 className="font-display font-bold text-slate-900 dark:text-white text-lg mb-1 leading-snug" style={{ fontFamily: "'Syne', sans-serif" }}>
                      {milestone.title}
                    </h3>
                    <h4 className="text-blue-600 dark:text-blue-400 font-medium text-xs mb-3">
                      {milestone.subtitle}
                    </h4>
                    <p className="text-slate-600 dark:text-white/60 text-xs font-light leading-relaxed">
